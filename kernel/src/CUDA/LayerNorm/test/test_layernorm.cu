...
template<typename T>
__global__ void layernorm_scalar(
		T *out, float *mean_out, float *rstd_out,
	...
	if (threadIdx.x == 0) { mean_out[bt] = mean; rstd_out[bt] = rstd; }
	for (int c = threadIdx.x; c < C; c += blockDim.x) {
		float v = static_cast<float>(x[bt * C + c]);
...
void test_type(const char *type_name) {
	...
	for (int ci = 0; ci < num; ++ci) {
		...
		layernorm_cpu(h_ref_out, h_ref_mean, h_ref_rstd,
				h_x, h_gamma, h_beta, BT, C, EPS);
		...
		TIME_MS(ms_scalar, 20, 200,
				(layernorm_scalar<T><<<BT, BLOCK>>>(
					d_out, d_mean, d_rstd, d_x, d_gamma, d_beta, C, EPS)));
		...
		// 读x + 读gamma + 读beta + 写out = (2*BT*C + 2*C) * sizeof(T)，C很小时gamma/beta可忽略
		...
		free(h_ref_out); free(h_ref_mean); free(h_ref_rstd);
		free(h_scalar); free(h_vec); free(h_mean); free(h_rstd);
	}
}
