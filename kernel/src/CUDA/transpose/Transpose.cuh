...
__global__ void transpose(T *out, const T *in, int R, int C, int out_stride){
		...
		for (int i = 0; i < 32; i += 8) {
				...
				if (r < C && c < R) out[r * out_stride + c] = tile[threadIdx.x][threadIdx.y + i];
		}
}

template<typename T>
void launch_transpose_forward(T *out, const T *in, int R, int C, int out_stride) {
		dim3 block(32, 8);
...
extern "C" void transpose_forward_##name(                                     \
		InT *out, const InT *in, int R, int C, int out_stride                 \
) {                                                                           \
