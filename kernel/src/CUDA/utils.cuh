        ...
        do { \
                ...
                if (_cuda_check_err != cudaSuccess) { \
                        fprintf(stderr, "CUDA error %s:%d: '%s' -> %s\n", \
                                        __FILE__, __LINE__, #call, cudaGetErrorString(_cuda_check_err)); \
                        exit(EXIT_FAILURE); \
                } \
        } while (0)
...
#define TIME_MS(out_ms, warmup, iters, ...) \
        do { \
                ...
                for (int _time_ms_i = 0; _time_ms_i < (warmup); ++_time_ms_i) { __VA_ARGS__; } \
                CUDA_CHECK(cudaDeviceSynchronize()); \
                CUDA_CHECK(cudaEventRecord(_time_ms_start_t)); \
                for (int _time_ms_i = 0; _time_ms_i < (iters); ++_time_ms_i) { __VA_ARGS__; } \
                CUDA_CHECK(cudaEventRecord(_time_ms_end_t)); \
                CUDA_CHECK(cudaEventSynchronize(_time_ms_end_t)); \
                float _time_ms = 0.0f; \
                CUDA_CHECK(cudaEventElapsedTime(&_time_ms, _time_ms_start_t, _time_ms_end_t)); \
